// backend/server.js - jspdf-autotable HATASI DÜZELTİLMİŞ FİNAL VERSİYON

const express = require('express');
const axios = require('axios');
const cors = require('cors');
const { jsPDF } = require("jspdf");
// !!!!!!! İŞTE DÜZELTME BURADA (1/2) !!!!!!!
// autoTable'ı ayrı bir değişken olarak alıyoruz.
...
app.get('/api/customer/:id/pdf', async (req, res) => {
    ...
    try {
        ...
        const query = `
          query GetCustomerById($merchantId: StringFilterInput!, $customerId: StringFilterInput!) {
            listCustomer(merchantId: $merchantId, id: $customerId) {
              data { id, fullName, email, phone, orderCount, totalOrderPrice, note, firstOrderDate, lastOrderDate, addresses { title, addressLine1, addressLine2, postalCode, district { name }, city { name }, country { name }, isDefault } }
            }
          }
        `;
        ...
        // !!!!!!! İŞTE DÜZELTME BURADA (2/2) !!!!!!!
        // autoTable fonksiyonunu doğrudan çağırıyoruz: autoTable(doc, options)
        autoTable(doc, {
            startY: 40,
            head: [['Alan', 'Değer']],
            body: customerInfo,
        ...
        console.error(`!!! PDF oluşturulurken KRİTİK HATA, müşteri ID: ${id}`, error.message, error.stack);
